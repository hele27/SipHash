...
int main()
{
	...
	myitoa(k); 
	strcat(knum,".txt");
	char filename[20] = "condtest";
	strcat(filename,knum);
	...
	for (int keycount=0;keycount<2*keynum;keycount++)
	{
		...
		if (flag==0)
			if (get_pos_i(h[2]^key1,0)!=0) key1 = flip_pos_i(key1,0);
		if (flag==1)
			if (get_pos_i(h[2]^key1,0)!=1) key1 = flip_pos_i(key1,0);
		if ((flag!=0)&&(flag!=1)) printf("error!\n");
		//test
